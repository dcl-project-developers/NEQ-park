...
public class SceneToGlTFWiz : MonoBehaviour
{
    ...
    public string correctExportPath (string inputPath)
    {
       
        string correctString = inputPath.Replace("\\", "/");
	...
	public IEnumerator Export(string path, Preset presetAsset, bool buildZip, bool exportPBRMaterials, bool exportAnimation = true, bool doConvertImages = false)
	{
       ...
       //savedPath = correctExportPath(savedPath); //Esto lo he añadido yo antes de ver la excepción de share violation, lo dejo por si acaso
         
        Debug.Log("Texture Path: " + path);
        Debug.Log("Texture Path post getdirectory: " + savedPath);
