...
function signatureListRequest(newpage: number, callback = function(){}){
  if (newpage<requestInfo.nPages && requestInfo.bPendingRequest==false) {
    ...
    executeTask(async () => {
      try {
        let response = await fetch("https://lowpolyhub.com:3000/api/signatures/momusEx/1?skip="+requestInfo.page*LIMIT+"&limit="+LIMIT, {
...
function signBookRequest(address:string, name:string, callback = function(){}){
  if (requestInfo.bAlreadySigned==false) {
    ...
    executeTask(async () => {
      try {
        let response = await fetch("https://lowpolyhub.com:3000/api/signatures/momusEx/1?address="+address+"&name="+name, {
          headers: { "Content-Type": "application/json" },
          method: "POST"
        })
        let json = await response.json()
...
export class SignPanel {
  ...
  constructor(entity: IEntity, panelEntity: IEntity, nextButton: IEntity, backButton: IEntity, pagesEntity: IEntity) {
    ...
    this.pagesEntity.addComponent(new Transform({position: position.add(positionPages), scale: new Vector3(0.9,0.9,0.9), rotation: tra.rotation.clone()}))
    ...
    this.pagesTextShape.color = Color3.White()
    this.pagesTextShape.value = (requestInfo.page+1)+" / "+requestInfo.nPages
    this.pagesEntity.addComponent(this.pagesTextShape)
    this.pagesEntity.getComponent(Transform).rotate(Vector3.Up(), 0)

    this.panelEntity.getComponent(Transform).translate((new Vector3(0,0.8,-0.1)))
    this.panelEntity.getComponent(Transform).rotate(Vector3.Up(), 0)

    const rows = new Entity()
    rows.addComponent(new Transform({scale:new Vector3(0.55,0.55,0.55)}))
    rows.setParent(this.panelEntity)

    const row1 = new Entity()
    row1.addComponent(new Transform({position: new Vector3(-2.8,0,0)}))
    row1.setParent(rows)
    const row2 = new Entity()
    row2.addComponent(new Transform({position: new Vector3(0,0,0)}))
    row2.setParent(rows)
    const row3 = new Entity()
    row3.addComponent(new Transform({position: new Vector3(2.8,0,0)}))
    row3.setParent(rows)
    ...
    this.panelTextShape1.color = Color3.White()
    ...
    this.panelTextShape2.color = Color3.White()
    ...
    this.panelTextShape3.color = Color3.White()
