...
export class WidgetTextTimmerCountdown extends WidgetTextTimmer{
  ...
  update(){
    if (this.countdownTime>(Date.now() - this.startTime)) {
      this.textUI.value = millisToMinutesAndSeconds(this.countdownTime - (Date.now() - this.startTime))
      let self = this
      this.timeout = setTimeout(() => {
        if (self.bUpdate && self.container.visible) {
          self.update()
      ...
      this.textUI.value = millisToMinutesAndSeconds(0)
    }
  }
}
